...
public class SearchGUI extends JFrame {
	//VARIABLES TO CHOOSE:
	double weightPopularity = 0.2;
	double alpha = 1;
	double beta = 0.5;
	double thresholdProbability = -6;
	double[] popularityScores ={0.3, 0.3, 0.3};
	int distanceFrames = 1;
	//For ranked retrieval
	boolean optimization = false;    
	double idf_threshold = 2;
	//For additional info (title, description,...)
	boolean addition = true;
	double weight_addition = 10;
			...
			for ( int i=0; i<results.size(); i++ ) {
				...
				buf.append( "" + results.get(i).docID );
					...
					if (results.get(i).getSizePos() > 1 ) {
						...
						if (indexer.index.docTimeFrame.get("" + results.get(i).docID) != null) {
							lengthFrame = indexer.index.docTimeFrame.get("" + results.get(i).docID);
						}
